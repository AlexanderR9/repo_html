...
//	4. burn (сжигание позы)
//
//ARG_1 - один обязательный аргумент - json file с полями для совершения транзакции.
// набор полей и их значения входного json_file зависят от типа транзакции (increase/decrease/collect/burn).
// 1-е обязательное поле должно быть "tx_kind" со значением - тип операции (increase/decrease/collect/burn).
...
/*
{
    ...
    // ----------- пример полей для операции "collect/burn"  ---------------------
    //нужен только pid
...
{
    if (v == "mint" || v == "increase" || v == "decrease" || v == "collect" || v == "burn") return "ok";
...
async function makeBurn() //return err_string or ""
{
    let err = "";
    log("burn position...");
    try 
    {
	const tx_result = await liq_worker.tryBurn(PID);
        log("tx_result:", tx_result);
	if (!IS_SIMULATE) 
	{
    	    if (hasField(tx_result, "tx_hash")) 
