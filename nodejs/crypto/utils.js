...
function priceToStr(p, is_stable_pool = false) //приводит вещественное значение цены в строку для пользователя, округляя до нужного количества знаков
{
    if (p <= 0) return p.toString();
    if (is_stable_pool) return p.toFixed(8);

    if (p < 0.1) return p.toFixed(6);
    if (p < 1.1) return p.toFixed(4);
